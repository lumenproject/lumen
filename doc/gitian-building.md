...
*Setup instructions for a Gitian build of Lumen Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lumen
...
- [Building Lumen Core](#building-lumen-core)
...
Clone the git repositories for Lumen Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/lumencoin/lumen
```
...
in the Lumen Core repository under 'Fetch and build inputs' to install sources which require
...
To build Lumen Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Lumen Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lumen/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lumencoin/lumen
...
For example:
```bash
URL=https://github.com/crowning-/lumen.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lumen=${COMMIT} --url lumen=${URL} ../lumen/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lumen=${COMMIT} --url lumen=${URL} ../lumen/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lumen=${COMMIT} --url lumen=${URL} ../lumen/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lumen git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lumen/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lumen.git
SIGPATH=/some/root/path/lumen-detached-sigs.git

./bin/gbuild --url lumen=${BTCPATH},signature=${SIGPATH} ../lumen/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lumen-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lumen-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lumen-osx-build.assert
```
...
[lumen/gitian.sigs](https://github.com/lumencoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
